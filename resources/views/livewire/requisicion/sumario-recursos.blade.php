<div>
    ...
    <div class="flex justify-end mb-6">
        <x-button wire:click="$set('showCrearRequisicionModal', true)" class="w-full sm:w-auto justify-center">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 mb-4">
            <thead class="bg-zinc-50 dark:bg-zinc-700">
                <tr>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[20%]">
                        Recurso</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[20%]">
                        Actividad</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[20%]">
                        Proceso Compra</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[10%]">
                        Unidad</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[20%]">
                        Detalles</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase w-[10%]">
            ...
            <tbody>
                @forelse($recursosSeleccionados as $recurso)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                        ...
                        <td class="px-3 py-2 text-zinc-600 dark:text-zinc-400">
                            <div class="text-sm">
                                <p><span class="font-semibold">Cantidad:</span> {{ $recurso['cantidad_seleccionada'] ?? '-' }}</p>
                                <p><span class="font-semibold">Precio Unitario:</span> L {{ number_format($recurso['precio_unitario'] ?? 0, 2) }}</p>
                                <p><span class="font-semibold">Total:</span> L {{ number_format($recurso['total'] ?? 0, 2) }}</p>
                            </div>
                        </td>
                        <td class="px-3 py-2">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-3 py-2 text-center text-zinc-500 dark:text-zinc-400">
        ...
        <x-slot name="content">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <x-input wire:model="descripcion" type="text" placeholder="Descripción de la requisición"
                        class="w-full" />
                    @error('descripcion')
                ...
                <div>
                    ...
                    <x-input wire:model="observacion" type="text" placeholder="Observación (opcional)"
                        class="w-full" />
                </div>
            </div>
        </x-slot>
