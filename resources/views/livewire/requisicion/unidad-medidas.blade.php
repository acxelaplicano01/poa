<div>
        ...
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    ...
                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar unidades..."
                ...
                <x-table
                    ...
                    empty-message="No hay unidades de medida disponibles"
                    class="mt-6"
                >
                    <x-slot name="desktop">
                        @forelse($unidadMedidas as $unidad)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                    <div class="font-medium">{{ $unidad->nombre }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                        ...
                        @forelse($unidadMedidas as $unidad)
                            <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700">
                                ...
                                <p class="text-zinc-600 dark:text-zinc-400 text-sm">
                                    Creado: {{ $unidad->created_at->format('d/m/Y H:i') }}
                                </p>
                            </div>
                        @empty
        ...
        <x-slot name="content">
            <div class="space-y-4">
                <div>
                    ...
                    <x-input id="nombre" class="block mt-1 w-full" wire:model="nombre"
                        placeholder="Ej: Kilogramos, Metros, Unidades" />
    ...
    <x-confirmation-modal maxWidth="md" wire:model="confirmingDelete">
        ...
        <x-slot name="content">
            ¿Está seguro de que desea eliminar la unidad de medida <span class="font-semibold">{{ $unidadAEliminar }}</span>? Esta acción no se puede deshacer.
        </x-slot>
