...
<div>
    <div class="mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Administración de Unidades Ejecutoras') }}</h2>
                ...
                <x-slot name="desktop">
                    @forelse ($unidades as $unidad)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <td class="px-6 py-4 text-zinc-900 dark:text-zinc-300 max-w-md truncate">
                                {{ optional($unidad->institucion)->nombre ?? 'Sin institución' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <button wire:click="edit({{ $unidad->id }})"
                ...
                <x-slot name="mobile">
                    @forelse ($unidades as $unidad)
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 mb-4">
                            ...
                            <h3 class="font-semibold text-zinc-900 dark:text-zinc-200 text-lg mb-2">{{ $unidad->name }}</h3>
                            <p class="text-zinc-600 dark:text-zinc-400 text-sm line-clamp-3">
                                {{ $unidad->descripcion ?: 'Sin descripción' }}
                            </p>
                            <p class="text-zinc-600 dark:text-zinc-400 text-sm">
                                <span class="font-semibold">{{ __('Estructura:') }}</span> {{ $unidad->estructura ?: 'Sin estructura' }}
                            </p>
                            <p class="text-zinc-600 dark:text-zinc-400 text-sm">
                                <span class="font-semibold">{{ __('Institución:') }}</span> {{ optional($unidad->institucion)->nombre ?? 'Sin institución' }}
                            </p>
                        </div>
                    @empty
    ...
    <!-- Modal para crear/editar unidad ejecutora -->
    @include('livewire.unidad-ejecutora.create')

    <!-- Modal de confirmación para eliminar -->
    @include('livewire.unidad-ejecutora.delete-confirmation')
