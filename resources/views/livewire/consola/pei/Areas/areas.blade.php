<div>
    <div class="mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Administración de Áreas') }}</h2>

                <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                    <div class="relative w-full sm:w-auto">
                        <x-input wire:model.live="search" type="text" placeholder="Buscar áreas..." class="w-full pl-10 pr-4 py-2"/>
            ...
            <x-table
                ...
                empty-message="{{ __('No se encontraron áreas')}}"
                class="mt-6"
            >
                <x-slot name="desktop">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                ...
                                <a href="{{ route('resultados', ['pei' => $peiId, 'area' => $area->id]) }}" 
                                   class="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                                    {{ $area->resultados->count() }} Resultados
                                </a>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @can('consola.areas.editar')
                ...
                <x-slot name="mobile">
                    @forelse ($areas as $area)
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 mb-4">
                            ...
                            <h3 class="font-semibold text-zinc-900 dark:text-zinc-200 text-lg mb-2">{{ $area->nombre }}</h3>
    ...
    @include('livewire.consola.pei.Areas.create')

    <!-- Modal de confirmación para eliminar -->
    @include('livewire.consola.pei.Areas.delete-confirmation')
