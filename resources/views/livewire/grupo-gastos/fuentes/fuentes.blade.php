<div>
    <div class=" mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Administración de Fuentes')}}
                </h2>

                <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                    <div class="relative w-full sm:w-auto">
                        <x-input wire:model.live="search" type="text" placeholder="Buscar fuentes..."
                    ...
                    @can('configuracion.fuentes.crear')
            ...
            <x-table
                ...
                empty-message="{{ __('No se encontraron fuentes')}}"
                class="mt-6"
            >
                <x-slot name="desktop">
                    @forelse ($fuentes as $fuente)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                <span class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-200">
                                    {{ $fuente->identificador }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @can('configuracion.fuentes.editar')
                                    <button wire:click="edit({{ $fuente->id }})"
                                    ...
                                    @endcan
                                    @can('configuracion.fuentes.eliminar')
                                    <button wire:click="confirmDelete({{ $fuente->id }})"
                ...
                <x-slot name="mobile">
                    @forelse ($fuentes as $fuente)
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 mb-4">
                            ...
                            <h3 class="font-semibold text-zinc-900 dark:text-zinc-200 text-lg mb-1">{{ $fuente->nombre }}</h3>
                            <div class="mt-2">
    ...
    @include('livewire.grupo-gastos.fuentes.createFuente')

    <!-- Modal de confirmación para eliminar -->
    @include('livewire.grupo-gastos.fuentes.delete-confirmation-Fuente')
