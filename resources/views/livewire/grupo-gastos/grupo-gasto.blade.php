<div>
    <div class=" mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Administración de Grupos de Gastos')}}
                </h2>

                <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                    <div class="relative w-full sm:w-auto">
                        <x-input wire:model.live="search" type="text" placeholder="Buscar grupos de gastos..."
                    ...
                    <div class="w-full sm:w-auto">
                     ...
                     @can('configuracion.grupogastos.crear')
            ...
            <x-table
                ...
                :columns="[
                    ...
                    ['key' => 'identificador', 'label' => 'Identificador', 'sortable' => true],
                    ['key' => 'actions', 'label' => 'Acciones'],
                ]"
                empty-message="{{ __('No se encontraron grupos de gastos')}}"
                class="mt-6"
            >
                <x-slot name="desktop">
                    @forelse ($grupoGastos as $grupo)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                <span class="bg-purple-100 text-purple-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-200">
                                    {{ $grupo->identificador }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @can('configuracion.grupogastos.editar')
                                    ...
                                    @endcan
                                    @can('configuracion.grupogastos.eliminar')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-zinc-500 dark:text-zinc-400">
                                {{ __('No se encontraron grupos de gastos')}}
                            </td>
                        </tr>
                    @endforelse
                </x-slot>
                ...
                <x-slot name="mobile">
                    @forelse ($grupoGastos as $grupo)
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 mb-4">
                            ...
                            <h3 class="font-semibold text-zinc-900 dark:text-zinc-200 text-lg mb-1">{{ $grupo->nombre }}</h3>
                            <div class="mt-2">
                                <span class="bg-purple-100 text-purple-800 text-sm font-medium px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-200">
                                    {{ $grupo->identificador }}
                                </span>
                            </div>
                        </div>
    ...
    <!-- Modal para crear/editar grupo de gastos -->
    @include('livewire.grupo-gastos.create-grupo-gasto')
