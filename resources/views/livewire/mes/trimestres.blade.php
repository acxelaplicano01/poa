<div>
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">
                        {{ __('Administración de Trimestres')}}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar trimestres..."
                        ...
                        @can('configuracion.trimestres.crear')
                ...
                <x-table
                    ...
                    :columns="[
                        ...
                        ['key' => 'trimestre', 'label' => 'Trimestre', 'sortable' => true],
                        ['key' => 'created_at', 'label' => 'Creado', 'sortable' => true],
                        ['key' => 'actions', 'label' => 'Acciones', 'class' => 'text-right'],
                    ]"
                    empty-message="No hay trimestres disponibles"
                    class="mt-6"
                >
                    <x-slot name="desktop">
                        @forelse($trimestres as $trimestre)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                    {{ $trimestre->created_at->format('d/m/Y H:i') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        @can('configuracion.trimestres.editar')
                                        <button wire:click="edit({{ $trimestre->id }})"
                                        ...
                                        @endcan
                                        @can('configuracion.trimestres.eliminar')
                                        <button wire:click="confirmDelete({{ $trimestre->id }})"
                        ...
                        @forelse($trimestres as $trimestre)
                            <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700">
                                ...
                                <p class="text-zinc-600 dark:text-zinc-400 text-sm">
                                    Creado: {{ $trimestre->created_at->format('d/m/Y H:i') }}
                                </p>
                            </div>
                        @empty
    ...
    <x-dialog-modal maxWidth="md" wire:model="isOpen">
        <x-slot name="title">
            <h3 class="text-lg font-medium text-zinc-900 dark:text-white">
                {{ $isEditing ? 'Editar Trimestre' : 'Nuevo Trimestre' }}
            </h3>
        </x-slot>
        ...
        <x-slot name="content">
            <div class="space-y-4">
                <div>
                    <x-label for="trimestre" value="Trimestre" />
                    <x-input id="trimestre" class="block mt-1 w-full" wire:model="trimestre"
                        placeholder="Ej: Trimestre 1 - 2024" />
                    @error('trimestre') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
            </div>
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-spinner-button 
                ...
                {{ $isEditing ? 'Actualizar Trimestre' : 'Crear Trimestre' }}
            </x-spinner-button>
            </div>
        </x-slot>
    </x-dialog-modal>
    ...
    <x-confirmation-modal maxWidth="md" wire:model="confirmingDelete">
        ...
        <x-slot name="content">
            ¿Está seguro de que desea eliminar este trimestre? Esta acción no se puede deshacer.
        </x-slot>
