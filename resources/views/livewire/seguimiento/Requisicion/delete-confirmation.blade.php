...
<x-dialog-modal wire:model="showDeleteModal" maxWidth="md">
            ...
            @if($requisicionToDelete)
                <div class="mt-2 p-4 bg-zinc-50 dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 flex items-center gap-3">
                    ...
                    <span class="text-lg font-semibold text-zinc-800 dark:text-zinc-100">
                        {{ $requisicionToDelete->descripcion ?? $requisicionToDelete->nombre ?? '-' }}
                    </span>
                </div>
            @endif
    ...
    <x-slot name="footer">
        <x-spinner-secondary-button wire:click="closeDeleteModal" loadingTarget="closeDeleteModal">
            Cancelar
